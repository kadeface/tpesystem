...
class LoadDataCommand(DataProcessingCommand):
    """加载数据命令"""
    ...
    def execute(self, context: dict) -> pd.DataFrame:
        ...
        Returns:
            加载的数据
        """
        return load_excel_file(self.file_path)

class ProgressAnalysisCommand(DataProcessingCommand):
    """进步分析命令"""
    def __init__(self, df_a: pd.DataFrame, subject_fields: dict):
        """
        初始化进步分析命令
        
        Args:
            df_a: 基准期数据
            subject_fields: 学科字段配置字典
        """
        self.df_a = df_a
    ...
    def execute(self, context: dict) -> pd.DataFrame:
        ...
        Args:
            context: 必须包含 current_term_data 键
            
        Returns:
            各学科进步分析结果字典
        """
        df_b = context['current_term_data']
        results = {}
        
        # 配置算法参数
        config = {
            'special_school_id': 78319,
            'weights': {'current':0.4, 'progress':0.6}
        }
        
        # 按学科处理
        for subject, fields in self.subject_fields.items():
            # 提取当前学科字段（排除教师字段）
            score_fields = fields[:-1] if len(fields) > 4 else fields
            
            # 初始化计算器
            calculator = ProgressCalculator(
                self.df_a[score_fields], 
                df_b[score_fields], 
                config
            )
            
            # 存储结果
            results[subject] = calculator.calculate_progress()
            
        return results

class ExportResultCommand(DataProcessingCommand):
    """导出结果命令"""
...
def main():
    ...
    try:
        ...
        # 初始化命令链
        commands = {
            "上学期处理": LoadDataCommand(FILE_A),
            "本学期处理": LoadDataCommand(FILE_B),
            "进步分析": ProgressAnalysisCommand(df_a, subject_fields),
            "结果导出": ExportResultCommand(OUTPUT_FILE)
        }
        ...
        try:
            # 处理基准数据
            cmd = commands["上学期处理"]
            df_a = cmd.execute(context)
            
            # 处理当前数据
            cmd = commands["本学期处理"]
            df_b = cmd.execute(context)
            context['current_term_data'] = df_b
            
            # 执行进步分析
            cmd = commands["进步分析"]
            context['results'] = cmd.execute(context)
